...
function getItemSubtotal(item) {
  ...
  return Number(item.price || item.priceSnap || item.product?.price || 0) *
...
function ShippingBadge({ status }) {
  const styles = {
    "sedang dikirim":   { bg: "#DBEAFE", color: "#1D4ED8" },
    "sampai di tujuan": { bg: "#D1FAE5", color: "#065F46" },
    "dikirim balik":    { bg: "#FEE2E2", color: "#DC2626" },
  };
    ...
    <div style={{ display: "flex", gap: 16, paddingBottom: isLast ? 0 : 20, position: "relative" }}>
      {!isLast && (
        <div style={{
          ...
          width: 2, height: "calc(100% - 4px)", background: "#E5E7EB",
        }} />
      )}
      <div style={{
  ...
  if (status === "cancelled" || status === "canceled") {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0" }}>
        ...
        <div>
          <p style={{ margin: 0, fontWeight: 600, fontSize: 13, color: "#DC2626" }}>
            Pesanan Dibatalkan
          </p>
        ...
        {loading ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[1,2,3].map(i => (
              <div key={i} style={{
                background: "#fff", borderRadius: 16,
                border: "1px solid #EBEBEB", padding: 20,
              }}>
          ...
          <SectionCard>
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              <p style={{ fontSize: 13, color: "#9CA3AF" }}>
                Cek kembali ID pesanan atau hubungi support.
              </p>
            </div>
          </SectionCard>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {items.length === 0 ? (
                ...
                <div>
                  {items.map((item, index) => (
                    <div key={item.order_item_id ?? item.id ?? index} style={{
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {shippingStatuses[item.order_item_id ?? item.id] && (
                          <ShippingBadge status={shippingStatuses[item.order_item_id ?? item.id]} />
                        )}
                      </div>
            ...
            {(canCancel || canConfirm) && (
              <div style={{
                display: "grid",
                gridTemplateColumns: canCancel && canConfirm ? "1fr 1fr" : "1fr",
                gap: 10,
              }}>
                {canCancel && (
