...
const getTasks = async (kurirId) => {
  ...
  return assignments.map((a) => ({
    ...
    store_name: a.orderItem.seller?.full_name || "-",
    pickup_address: `${a.orderItem.seller?.full_name || "Toko"}, ${a.orderItem.order?.address?.address || ""}`,
    delivery_address: `${a.orderItem.order?.address?.address || ""}, ${a.orderItem.order?.address?.city || ""}`,
    buyer_name: a.orderItem.order?.buyer?.full_name || "-",
